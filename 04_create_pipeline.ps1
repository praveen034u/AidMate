...
Write-Host "=== Create test_pipeline.ps1 ===" -ForegroundColor Cyan
...
$pipelinePath = Join-Path $Workspace "test_pipeline.ps1"
Set-Content -Path $pipelinePath -Value $pipeline -Encoding UTF8
Write-Host "[OK] Created: $pipelinePath" -ForegroundColor Green

if ($Run) {
  Write-Host "[INFO] Running pipeline..." -ForegroundColor Yellow
  powershell -ExecutionPolicy Bypass -File $pipelinePath
}
