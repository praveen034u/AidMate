...
from langchain_ollama import OllamaEmbeddings as _OllamaEmbeddingsDuplicateGuard  # guard
...
# ------------- LangChain per-session memory -------------
LC_HISTORY_STORE = {}

def _get_lc_history(session_id: str) -> ChatMessageHistory:
    key = session_id or "_default"
    if key not in LC_HISTORY_STORE:
        LC_HISTORY_STORE[key] = ChatMessageHistory()
    return LC_HISTORY_STORE[key]

def _history_resolver(config):
    # config may be a dict or other; be defensive
    try:
        if isinstance(config, dict) and "configurable" in config:
            sid = config["configurable"].get("session_id", "_default")
        else:
            sid = "_default"
    except Exception:
        sid = "_default"
    return _get_lc_history(sid)
VECTOR_DB_ROOT.mkdir(parents=True, exist_ok=True)

# -------------------------
# Global caches to speed up requests
# -------------------------
_EMBEDDINGS = None            # OllamaEmbeddings singleton
_VECTORSTORE = None           # FAISS store (loaded once, reused)
_RETRIEVER = None             # vectorstore.as_retriever(...)
_CHAIN_CACHE = {}             # {(model): RunnableWithMessageHistory}
...
def _load_vector_store():
    """Load FAISS vectorstore once and memoize. Refresh if files updated."""
    global _VECTORSTORE, _RETRIEVER, _CHAIN_CACHE
    if _VECTORSTORE is None:
        emb = _get_embeddings()
        _VECTORSTORE = FAISS.load_local(VECTOR_DB_ROOT, emb, allow_dangerous_deserialization=True)
        _RETRIEVER = _VECTORSTORE.as_retriever(search_kwargs={"k": 3})
        _CHAIN_CACHE.clear()  # invalidate chains because they capture the old retriever
    return _VECTORSTORE

def _invalidate_vector_cache():
    """Call this after you rebuild the FAISS index from uploads."""
    global _VECTORSTORE, _RETRIEVER, _CHAIN_CACHE
    _VECTORSTORE = None
    _RETRIEVER = None
    _CHAIN_CACHE.clear()
...
def _get_or_build_chain(model: str):
    """Cache a memory-enabled RAG chain per model using the shared retriever."""
    ch = _CHAIN_CACHE.get(model)
    if ch is not None:
        return ch
    ...
    - If the question is not related to the context, politely insist that you are tuned to only answer questions related to the context.
    ...
    _CHAIN_CACHE[model] = chain_with_memory
...
def ask(body: AskBody, x_session_id: Optional[str] = Header(default="")):
    ...
    reply = call_Vector_ollama(body.prompt, body.model, x_session_id)
    print("Reply:", reply)
...
def setup_rag_chain(model, vector_store, humanMessage: str):
    # vector_store arg kept for compatibility; we use shared cached retriever for speed
    return _get_or_build_chain(model)

def call_Vector_ollama(full_prompt: str, model: str, session_id: str = "") -> str:
    # Ensure vector store is loaded (memoized)
    _load_vector_store()
    chain = _get_or_build_chain(model)
    response = chain.invoke(
        {"question": full_prompt},
        config={"configurable": {"session_id": session_id or "_default"}}
    )
    return response
...
def build_plain_chat_chain_with_memory(model: str, system_text: str = ""):
