...
# Defaults only if not already defined earlier in the script
$ModelAlias_OptionA = if (Test-Path variable:ModelAlias_OptionA) { $ModelAlias_OptionA } else { "gpt-oss:latest" }
$BackModel_OptionA  = if (Test-Path variable:BackModel_OptionA)  { $BackModel_OptionA  } else { "phi3:mini" }
$ApiPort_OptionA    = if (Test-Path variable:ApiPort_OptionA)    { $ApiPort_OptionA    } else { 8000 }
$CpuThreads_OptionA = if (Test-Path variable:CpuThreads_OptionA) { $CpuThreads_OptionA } else { 4 }
...
function Ensure-GPTOssAlias {
  param([string]$OllamaExe, [string]$ModelAlias, [string]$BackModel)

  # SAFE TEMP HANDLING
  $tempRoot = if ($env:TEMP -and $env:TEMP.Trim()) { $env:TEMP } else { [System.IO.Path]::GetTempPath() }
  $ModelfilePath = Join-Path $tempRoot "Modelfile_gpt_oss.txt"
...
try {
  ...
  Set-CPUTuning -Threads $CpuThreads_OptionA -ApiPort $ApiPort_OptionA
} catch {
